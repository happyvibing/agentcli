...
test("daemon start is idempotent", () => {
  const r = cli(["daemon", "start"]);
  assert.equal(r.status, 0, r.stderr);
  assert.equal(JSON.parse(r.stdout as string).alreadyRunning, true);
});
...
test("tool errors keep their exit codes through the daemon", () => {
  ...
  assert.equal(JSON.parse(nf.stderr as string).error.code, "NOT_FOUND");
});

test("--no-daemon flag forces the direct path", () => {
  const r = cli(["demo", "echo", "--message", "x", "--no-daemon"]);
  assert.equal(r.status, 0, r.stderr);
  assert.equal(JSON.parse(r.stdout as string).meta.via, "direct");
});
...
test("after daemon stop, calls transparently fall back to direct", () => {
  ...
  assert.equal(JSON.parse(stopped.stdout as string).stopped, true);

  const status = cli(["daemon", "status"]);
  assert.equal(JSON.parse(status.stdout as string).running, false);
...
test("daemon restarts cleanly", () => {
  const started = cli(["daemon", "start"]);
  assert.equal(started.status, 0, started.stderr);
  const r = cli(["demo", "echo", "--message", "restarted"]);
  assert.equal(r.status, 0, r.stderr);
  assert.equal(JSON.parse(r.stdout as string).meta.via, "daemon");
  const stopped = cli(["daemon", "stop"]);
  assert.equal(stopped.status, 0, stopped.stderr);
});
